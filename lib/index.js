...
module.exports = function (babel) {   
  ...
  const visitor = {
    AwaitExpression(path) {
      ...
      var catchStatement = template.statement(`var tmp = ${options.catchFn.toString()}`)();
      var CatchFn = catchStatement.declarations[0].init.body; 

      // 通过temp创建try语句
      let tryNode = temp([CatchError, CatchFn.body]);
