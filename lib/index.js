...
module.exports = function (babel) {
  ...
  const visitor = {
    AwaitExpression(path) {
      ...
      // };
      const CatchError = types.stringLiteral(catchConsole(filePath, funcName, options.customLog))
      // 将配置文件传进来的 catchFn 转为AST节点 然后从AST节点中获取函数体 作为catch里面的内容
      const catchStatement = template.statement(`var tmp = ${options.catchFn.toString()}`)()
      const CatchFn = catchStatement.declarations[0].init.body

      // 通过temp创建try语句
      const tryNode = temp([CatchError, CatchFn.body])
  ...
  return {
    name: 'babel-plugin-await-add-customCatch',
    visitor,
  }
}
